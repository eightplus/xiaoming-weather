...
ForecastTip::ForecastTip(QWidget *parent) :
    QFrame(parent)
    , m_radius(0)
...
void ForecastTip::resetData(const ForecastWeather &data, const QString &week)
{
    m_dateLabel->setText(QString("%1 %2").arg(week).arg(data.forcast_date));

    QPixmap pixmap1 = QPixmap(":/res/101.png");
//    QPixmap pixmap1 = QPixmap(QString(":/res/weather_icons/darkgrey/%1.png").arg(data.cond_code_d));
    ...
    QPixmap pixmap2 = QPixmap(":/res/101.png");
...
void ForecastTip::popupTip(QPoint point)
{
    int point_X;
    int point_Y;
...
void ForecastTip::paintEvent(QPaintEvent *event)
{
